// model/analysis_model.dart — Modelos de datos del análisis biomecánico

// ── Estado de validación de video (pre-grabación) ────────────────────────────
enum VideoValidationStatus { idle, validating, valid, invalid }

class VideoValidationModel {
  final VideoValidationStatus status;
  ...
  const VideoValidationModel({
    this.status = VideoValidationStatus.idle,
  ...
  bool get allPassed => durationOk && sizeOk && poseOk;

  static const VideoValidationModel empty = VideoValidationModel();
}
...
class AnalysisResultModel {
  ...
  final List<RepResultModel> repetitions;
  final List<FeedbackItemModel> feedback;
  final String? annotatedDownloadUrl;
  final DateTime sessionDate;

  const AnalysisResultModel({
    required this.sessionId, this.jobId, required this.exerciseType,
    required this.videoView, this.weightKg, required this.totalReps,
    required this.techniqueScore, required this.fatigueDetected,
    this.fatigueIndex = 0.0, this.durationSeconds,
    this.classifierEnabled = false, this.classifierVersion,
    required this.paramsSummary, required this.repetitions,
    required this.feedback, required this.aiComparison,
    this.annotatedDownloadUrl, required this.sessionDate,
  });

  factory AnalysisResultModel.fromJson(Map<String, dynamic> j) => AnalysisResultModel(
    ...
    repetitions: (j['repetitions'] as List? ?? []).map((r) => RepResultModel.fromJson(r)).toList(),
    feedback: (j['feedback'] as List? ?? []).map((f) => FeedbackItemModel.fromJson(f)).toList(),
  ...
  List<FeedbackItemModel> get riskFeedback    => feedback.where((f) => f.isInjuryRisk).toList();
  List<FeedbackItemModel> get improveFeedback => feedback.where((f) => !f.isInjuryRisk && f.reportSection == 'mejora').toList();
}

// ── Modelo de repetición individual ─────────────────────────────────────────
class RepResultModel {
  final int repNumber;
  final double repScore;
  final bool depthAchieved;
  final double? kneeAngleMin, hipAngleMin, trunkLeanMax;
  final double? leftValgus, rightValgus, ankleDF;
  final double? eccentricDur, concentricDur, eccConRatio, barVelocityMs;
  final double? kneeAsymmetryPct, lateralHipShiftCm, pelvicRotationDeg;
  final double? heelElevationLeft, aclRiskScore, patellofemoralLoad, lumbarRiskScore;
  ...
  const RepResultModel({
    required this.repNumber, required this.repScore, required this.depthAchieved,
    this.kneeAngleMin, this.hipAngleMin, this.trunkLeanMax, this.leftValgus,
    this.rightValgus, this.ankleDF, this.eccentricDur, this.concentricDur,
    this.eccConRatio, this.barVelocityMs, this.kneeAsymmetryPct,
    this.lateralHipShiftCm, this.pelvicRotationDeg, this.heelElevationLeft,
  ...
  factory RepResultModel.fromJson(Map<String, dynamic> j) => RepResultModel(
...
class FeedbackItemModel {
  ...
  final int frequency, frequencyPct, priorityRank;
  final bool isInjuryRisk;
  final String reportSection;

  const FeedbackItemModel({
    required this.errorType, required this.severity,
    required this.message, required this.correction,
    this.exerciseRecommendation, required this.frequency, this.frequencyPct = 0,
    required this.priorityRank, required this.isInjuryRisk,
    this.reportSection = 'mejora',
  });

  factory FeedbackItemModel.fromJson(Map<String, dynamic> j) => FeedbackItemModel(
    ...
    frequencyPct: j['frequency_pct'] ?? 0,
    priorityRank: j['priority_rank'] ?? 1,
    isInjuryRisk: j['is_injury_risk'] ?? false,
    reportSection: j['report_section'] ?? 'mejora',
  );
}
...
class OneRMModel {
  ...
  const OneRMModel({required this.weightKg, required this.reps,
      required this.oneRmEpley, required this.oneRmBrzycki,
      required this.oneRmAverage, required this.equivalences});

  factory OneRMModel.fromJson(Map<String, dynamic> j) => OneRMModel(
...
// ── Modelo sesión de entrenamiento (historial) ───────────────────────────────
class WorkoutSessionModel {
  ...
  const WorkoutSessionModel({
    required this.id, required this.exerciseType, this.jobId, this.weightKg,
    required this.totalReps, required this.totalSets, required this.techniqueScore,
    required this.fatigueDetected, this.classifierEnabled = false,
    required this.sessionDate,
  });

  factory WorkoutSessionModel.fromJson(Map<String, dynamic> j) => WorkoutSessionModel(
...
// ── Modelo del estado del modelo IA personal ─────────────────────────────────
class AIModelStateModel {
  ...
  const AIModelStateModel({
    required this.phase, required this.phaseMessage, required this.progress,
  ...
  factory AIModelStateModel.fromJson(Map<String, dynamic> j) => AIModelStateModel(
